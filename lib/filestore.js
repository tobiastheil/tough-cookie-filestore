...
function FileCookieStore(filePathRead, filePathWrite) {
    if (typeof filePathWrite == 'undefined') {
    	filePathWrite = filePathRead;
    }
    ...
    var self = this;
    loadFromFile(this.filePathRead, function(dataJson) {
        if(dataJson)
            self.idx = dataJson;
    })
}
...
FileCookieStore.prototype.putCookie = function(cookie, cb) {
    ...
    saveToFile(this.filePathWrite, this.idx, function() {
        cb(null);
    });
};
